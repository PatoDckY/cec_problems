...
class ProblemaC10:
    ...
    def generar_m(self, D, seed=None):
        if seed:
            np.random.seed(seed)
        A = np.random.randn(self.D, self.D)
        Q, _ = np.linalg.qr(A)
    ...
    def evaluate(self, individuo):
        ...
        self.y = (self.x - self.offset) @ self.m
        fitness = self.aptitud(self.z)
        suma_violaciones = self.sumar_violation(self.y)
        return fitness, suma_violaciones
    ...
    def h1(self, y):
         violation = np.sum(y * np.sin(np.sqrt(np.abs(y))))
         return violation
...
class ProblemaC11:
    ...
    def aptitud(self, z):
        return np.mean(-z * np.cos(2 * np.sqrt(np.abs(z))))
...
class ProblemaC13:
    ...
    def aptitud(self, z):
        return np.mean(-z * np.sin(np.sqrt(np.abs(z))))
...
class ProblemaC17:
    ...
    def sumar_violation(self, z):
        viol_g1 = max(0.0, self.g1(z))
        viol_g2 = max(0.0, self.g2(z))
        viol_h1 = max(0.0, abs(self.h1(z)) - self.tolerance)
